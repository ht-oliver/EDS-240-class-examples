...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = body_mass_g)) + 
  geom_point() +
  scale_color_gradient(low = '#f7dd4c', high = '#5ca0ff')
```
...
ggplot() +
  geom_point(data = penguins, mapping = aes(x = bill_length_mm,
                                            y= bill_depth_mm,
                                            color = species),
             alpha = 0.7) +
  geom_point(data = penguins_summary, mapping = aes(x = mean_bill_length_mm,
...
(p1 + (p2 / p3)) +
  plot_annotation(title = "Uuuuhgh ohmygod")
